...
def define_config_space_jemalloc():
    ...
    input_space.add(CS.UniformIntegerHyperparameter(name='je_oversize_threshold', lower=16*1024, upper=32*1024*1024)) # 32 MiB
    ...
    try:
        with open('./dump.log', 'w') as dump_file:
            result = subprocess.Popen(
                    ['numactl', '--cpunodebind=1', '--membind=1', 'runcpu', '--config=ap-new.cfg', '--action', 'onlyrun', f'--size={args.size}', '--tune=base', f'--threads={args.threads}', f'{args.benchmark}'],
...
def launch_benchmark_external(env_dict, args):
    new_env = os.environ.copy()
    new_env.update({"LD_PRELOAD": "/usr/local/lib/libjemalloc.so"})
    ...
    je_malloc_conf_str = ""
    je_malloc_conf_str += 'stats_print:true,'
    je_malloc_conf_str += 'confirm_conf:true,'
    for key, val in env_dict.items():
        if key.startswith('je_'):
            je_malloc_conf_str += f"{key.removeprefix('je_')}:{str(val)},"
...
def calculate_rss_score(current_rss, default_rss, interval=0.1):
    ...
    print(f'Length of input default_rss: {len(default_rss)}')

    current_arr = np.array(current_rss)
    default_arr = np.array(default_rss)
    
    print(f'Length of current rss array: {len(current_arr)}')
    print(f'Length of default rss array: {len(default_arr)}')
    # Determine max length between the two arrays
    max_length = max(len(current_arr), len(default_arr))
    
    # Pad shorter array with trailing zeros using numpy.pad
    if len(current_arr) < max_length:
        current_arr = np.pad(current_arr, (0, max_length - len(current_arr)), 'constant')
    if len(default_arr) < max_length:
        default_arr = np.pad(default_arr, (0, max_length - len(default_arr)), 'constant')
    
    # Calculate regret between padded arrays
    regret = current_arr - default_arr
    
    # Generate timestamps based on max length
    timestamps = np.arange(max_length) * interval
    
    # Store results (assuming df is a predefined DataFrame)
    df[f'run_{len(df.columns)-1}'] = pd.Series(current_arr)
